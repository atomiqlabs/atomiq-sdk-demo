...
async function main() {
    ...
    const dstToken = Tokens.BOTANIX.BTC;
    ...
    swap.wrapper.events.on("swapState", (swap) => {
        if(swap.getId()!==swapId) return;
    ...
    //     const txns = await swap.txsClaim(evmWallet);
    //     //Execute the transaction through the wallet (you can also execute them in other ways - e.g. use AA, etc.)
    //     for(let tx of txns) {
    //         await evmWallet.sendTransaction(tx);
    //     }
    // }
