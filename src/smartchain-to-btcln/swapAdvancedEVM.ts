...
import {evmWallet, starknetWallet} from "../wallets";

//Swap of EVM assets to Bitcoin L2 lightning network
...
async function main() {
    ...
    const srcToken = Tokens.CITREA.CBTC;
    ...
    if(!swapSuccessful) {
        ...
        // await swap.refund(evmWallet);
        //b. or you can obtain the required transactions and sign & broadcast them manually
        const txsRefund = await swap.txsRefund();
        for(let tx of txsRefund) {
            await evmWallet.signTransaction(tx);
        }
